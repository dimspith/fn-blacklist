...
const addListeners = () => {
  u(customAPISubmit).on("click", () => {
    ...
    bulmaToast.toast({
      message: "API URL submitted successfully!",
      type: "is-success",
    });
  });
  ...
  u(contributor).on("click", () => {
    if (u(contributor).is(":checked")) {
      u(tokenForm).removeClass("is-hidden");
      chrome.runtime.sendMessage({ message: "set-contributor", value: true });
    } else {
      u(tokenForm).addClass("is-hidden");
      chrome.runtime.sendMessage({ message: "set-contributor", value: false });
    }
  });
